    ...
    >
      <Card className="w-full rounded-xl shadow-lg sm:shadow-2xl shadow-gray-600/30 dark:shadow-md dark:shadow-gray-50/20 border border-gray-100 dark:border-gray-800 overflow-hidden">
        ...
        <div className="relative">
          ...
          <CardHeader className="pb-0 pl-6 sm:pl-6 pr-2 pt-2"> {/* Reduced padding */}
            <div className="flex justify-between items-start">
              <div>
                <CardTitle className="text-base sm:text-md font-bold"> {/* Smaller font */}
                  {title}
                </CardTitle>
                {description && (
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0"> {/* Reduced margin */}
                    {description}
                  </p>
                )}
              </div>
              {!isEditing && (
                <button 
                  ...
                  aria-label="Edit"
                >
                  <Pencil className="h-3 w-3 sm:h-4 sm:w-4" /> {/* Smaller icon */}
                </button>
              )}
            </div>
          </CardHeader>
        ...
        {avatar && (
          <div className="flex justify-center mt-1 mb-2"> {/* Reduced margin */}
            <div className="h-14 w-14 sm:h-16 sm:w-16 rounded-full"> {/* Smaller avatar */}
              <img 
                src={avatar} 
                alt="Profile" 
        ...
        <form onSubmit={handleSubmit}>
          <div className={`grid ${getGridClasses()} gap-2 px-3 sm:px-4 pb-1`}> {/* Reduced padding and gap */}
            {fields.map((field) => (
              <motion.div
                key={field.id}
                ...
                transition={{ duration: 0.3 }}
                className={`${field.gridSpan ? `md:col-span-${field.gridSpan}` : ''}`}
              >
                <CardContent className="space-y-0 p-0"> {/* Removed vertical space */}
                  <div className="grid grid-cols-12 items-center gap-1"> {/* Tightened gap */}
                    <div className="col-span-4 flex items-center gap-1"> {/* Tightened gap */}
                      {field.icon && (
                    ...
                    <div className="col-span-8">
                      {isEditing ? (
                        ...
                        <div className={`text-xs text-gray-700 dark:text-gray-300 px-1 py-0.5 ${
            ...
            {isEditing && emptySlots > 0 && (
              <div className={`md:col-span-${emptySlots} flex items-end justify-end pt-1`}> {/* Reduced padding */}
                <div className="flex gap-1 w-full justify-end"> {/* Tightened gap */}
                  <Button
                    type="button"
                    variant="outline"
          ...
          {isEditing && emptySlots === 0 && (
            <CardFooter className="flex flex-row justify-end gap-1 px-3 pb-2 pt-1 w-full"> {/* Reduced padding */}
